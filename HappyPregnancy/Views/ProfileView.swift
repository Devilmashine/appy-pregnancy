...
struct ProfileView: View {
    @EnvironmentObject var pregnancyInfoViewModel: PregnancyInfoViewModel
    @EnvironmentObject var measurementViewModel: MeasurementViewModel
    @EnvironmentObject var doctorQuestionViewModel: DoctorQuestionViewModel
    ...
    var body: some View {
        NavigationView {
            ScrollView {
                VStack(spacing: 20) {
                    // Профиль
                    ProfileHeaderView(info: pregnancyInfoViewModel.pregnancyInfo)
                    
                    // Графики
                    if !measurementViewModel.measurements.isEmpty {
                        MeasurementsChartView(measurements: measurementViewModel.measurements)
                    }
                    ...
                    TabView(selection: $selectedTab) {
                        ...
                        .environmentObject(measurementViewModel)
                        .tag(0)
                        
                        DoctorQuestionsView()
                            .environmentObject(doctorQuestionViewModel)
                            .tag(1)
            ...
            .sheet(isPresented: $showingEditProfile) {
                if let info = pregnancyInfoViewModel.pregnancyInfo {
                    EditProfileView(info: info)
        ...
        VStack {
            if viewModel.doctorQuestions.isEmpty {
                ...
                List {
                    ...
                    .onDelete(perform: viewModel.deleteDoctorQuestion)
                }
            }
        }
        .sheet(isPresented: $showingAddQuestion) {
            AddDoctorQuestionView()
